...
const rawSeoData: Omit<SeoPageData, 'summary' | 'faqs'>[] = [
  ...
  // --- BLOG POSTS ---
  {
    ...
    "FAQ Schema (JSON-LD)": "{}", "Product Schema (JSON-LD)": null, "LocalBusiness Schema (JSON-LD)": "{}", "Combined Schema (JSON-LD)": "{}", "CTA": "Read More", "Schema Type": "Article"
  },
  {
  ...
  // --- PRODUCTS ---
  {
    ...
    "Product Schema (JSON-LD)": "{}", "LocalBusiness Schema (JSON-LD)": "{}", "Combined Schema (JSON-LD)": "{}", "CTA": "Request a Quote", "Schema Type": "Product"
  },
  {
...
const categoryNameToIconName: { [key: string]: string } = {
    'Safety Tapes': 'ShieldCheckIcon', 'Reflective Tapes': 'SunIcon', 'Double-Sided Tapes': 'LayersIcon',
    'Specialty Tapes': 'SparklesIcon', 'PTFE & Teflon Tapes': 'ThermometerIcon', 'Antistatic & ESD Tapes': 'ZapIcon',
};

/**
 * A transformed list of all products, structured for use in the ProductContext.
 * This logic aligns with the user's specific request for mapping fields.
 */
...
export const ALL_BLOG_ARTICLES: BlogArticle[] = seoData
    ...
    .map((p, index) => {
        ...
        return {
            id,
            title: p.H1, // As per instruction
            summary: p["Meta Description (≤160 chars)"], // As per instruction
            content: p.summary, // As per instruction
            category: 'Industry Guides',
            tags: allKeywords.split(',').map(t => t.trim()), // As per instruction
            image: `/default-blog.jpg`, // As per instruction
            readTime: 5 + (index % 3),
